...
export async function getDerConfigurations(queryParams: DERQueryParams) {
  const response: RawPaginationSet<{ der_configurations: DERConfiguration[] }> = await getRequest(
...
export async function getDerStrategies(queryParams: DERQueryParams) {
  const response: RawPaginationSet<{ der_strategies: DERStrategy[] }> = await getRequest(
    routes.strategy(),
    queryParams
  ).then((res) => res.json());
