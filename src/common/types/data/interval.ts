...
type IntervalDataFilters = Partial<{
  month: MonthIndex,
  start: Date,
  end: Date,
  range: [Date, Date]
...
type ValueDomainTuple = [number, number];
type TimeDomainTuple = [Date, Date];
...
export class IntervalDataWrapper {
  ...
  static create<K extends string, V extends string> (
   ...
   * provided, the `IntervalDataWrapper` is returned unchanged. Several different types of filters
   * can be used:
   *
   ...
   * @param others
   */
  private align (...others: IntervalDataWrapper[]) {
  ...
  /**
   * Creates a new `IntervalDataWrapper` by calling a function on every datum within the series. The
   * timestamps will not be changed.
   *
   * @param {function} fn: the function to call for every datum
   * @param {string} [name]: the name of the resulting `IntervalDataWrapper`. Defaults to the
   ...
   *
   * @param {IntervalDataWrapper} other: the other interval dataset which will be subtracted
   */
  subtract (other: IntervalDataWrapper) {
    return new IntervalDataWrapper(
      omitFalsey(
   ...
   *   this will become the new `IntervalDataWrapper`'s value at the corresponding timestamp.
   * @param {string} [name]: the name of the resulting `IntervalDataWrapper`
   */
  ...
  /**
   * Produces an `IntervalDataWrapper` with the same domain, with values taken from a provided
   * `Frame288Numeric`
   *
