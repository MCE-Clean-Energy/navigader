...
describe('Frame288Numeric', () => {
  describe('`divide` method', () => {
    ...
    it('divides all values in the frame', () => {
      ...
      months.forEach((monthIndex) =>
        divided.getMonth(monthIndex).forEach((val, hour) => expect(val === hour))
      );
    });
  ...
  describe('`rename` method', () => {
    const originalFrame = new Frame288Numeric(fixtures.frame288, { name: 'original', units: 'kW' });
...
describe('PowerFrame288', () => {
  describe('`scale` method`', () => {
    function scale288WithMinAndMax(min: number, max: number) {
      return new PowerFrame288(makeFrame288(() => _.sample([min, max]) as number).frame).scale();
    }
