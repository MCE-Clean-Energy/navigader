...
describe('load hooks', () => {
  ...
  describe('`useMeterGroup` hook', () => {
    ...
    it('uses the API if the period filter does not match', async () => {
      ...
      const result = await testHook(store, useMeterGroup, [meterGroup15Mins], testMeterGroup.id, {
        period: 60,
      });
    ...
    it('skips the API if a meter group matching the filters is in the store', async () => {
      const result = await testHook(store, useMeterGroup, [testMeterGroup], testMeterGroup.id, {
        data_types: 'default',
        period: 60,
      });
