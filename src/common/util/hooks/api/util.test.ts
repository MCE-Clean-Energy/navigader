...
describe('API hook utilities', () => {
  describe('`applyDataFilters` helper', () => {
    ...
    it('applies single `data_types` filters appropriately', () => {
      ...
      expect(applyDataFilters({ data: {} }, { data_types: 'default' })).toBeFalsy();
      ...
      frame288Types.forEach((frame288Type) => {
        ...
        expect(applyDataFilters({ data: {} }, { data_types: frame288Type })).toBeFalsy();
      });
    });
    ...
    it('applies `period` filters appropriately', () => {
      expect(applyDataFilters({ data: {} }, { period: 60 })).toBeFalsy();

      expect(applyDataFilters({ data: { default: hourInterval } }, { period: 60 })).toBeTruthy();

      expect(applyDataFilters({ data: { default: hourInterval } }, { period: 15 })).toBeFalsy();
    });

    it('applies combined filters', () => {
      expect(
        applyDataFilters({ data: { default: hourInterval } }, { data_types: 'default', period: 60 })
      ).toBeTruthy();

      // wrong period
      expect(
        applyDataFilters({ data: { default: hourInterval } }, { data_types: 'default', period: 15 })
      ).toBeFalsy();
  ...
  describe('`applyDynamicRestFilters` helper', () => {
    ...
    it('applies multiple filters appropriately', () => {
      ...
      expect(
        applyDynamicRestFilters(complexModel, {
          filter: {
