...
import { getComputedValue, Meter, MeterGroup } from '@nav/shared/models/meter';
...
const MetersTable: React.FC<MetersTableProps> = ({ meterGroupId }) => {
          ...
          >
            <Table.Head>
              <Table.Row>
                ...
                <Table.Cell align="right">Rate Plan</Table.Cell>
                <Table.Cell align="right">Total kWh</Table.Cell>
                <Table.Cell align="right">Maximum kW</Table.Cell>
              </Table.Row>
            </Table.Head>
            <Table.Body>
              {meters.map(meter =>
                <Table.Row key={meter.id}>
                  ...
                  <Table.Cell align="right">{meter.metaData.ratePlan}</Table.Cell>
                  <Table.Cell align="right">abcd</Table.Cell>
                  <Table.Cell align="right">{getMaxKw(meter)}</Table.Cell>
                </Table.Row>
              )}
            </Table.Body>
          </Table>
  ...
  function getMaxKw (meter: Meter) {
    const maxKw = getComputedValue(meter, 'maxKw');
    return typeof maxKw === 'number'
      ? parseFloat(maxKw.toFixed(3))
      : '-';
  }
};
